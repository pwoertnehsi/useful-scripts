...
Param(
    [Alias("i")][ValidateScript({Test-Path $_ -PathType Leaf})][string]$InputFile,
    [Alias("o")][string]$OutputDir = ".\output",
    [Alias("q")][int]$Crf = 30,
    [Alias("t")][int]$Tile = 64,
    [Alias("w")][int]$OutW = 100,
    [Alias("h")][int]$OutH = 100,
    [int]$X = 10,
    [int]$Y = 10,
    [Alias("p")][int]$MaxProcesses = 4
)
...
function Render-Matrix {
    ...
    for ($i = 0; $i -le $VerTiles; $i++) {
        [void]$sb.Append("$([char]27)[2K`r") 
        for ($j = 0; $j -le $HorTiles; $j++) {
            $col = $Matrix["$i,$j"]
            [void]$sb.Append("$([char]27)[$($col)m■ $([char]27)[0m")
        }
        [void]$sb.Append("`n")
    }
    ...
    if ($script:DoneCount -gt 0) {
        $ETA = [TimeSpan]::FromSeconds((($TotalTiles - $script:DoneCount) * ($Elapsed.TotalSeconds / $script:DoneCount)))
        $Host.UI.RawUI.WindowTitle = "Rendering: $($script:DoneCount)/$TotalTiles | ETA: $($ETA.ToString('hh\:mm\:ss'))"
    }

    [void]$sb.Append("`n$([char]27)[2K$($script:DoneCount)/$TotalTiles`n")
...
try {
	Init-Matrix
	for ($i=0; $i -le ($VerTiles + 2); $i++) { [Console]::WriteLine("") }
	Render-Matrix 
    ...
    for ($curY=0; $curY -le $VerTiles; $curY++) {
        for ($curX=0; $curX -le $HorTiles; $curX++) {
            ...
            $si.Arguments = "-y -i `"$ResolvedInput`" -an -v quiet -vf `"$VFilter`" -c:v libvpx-vp9 -pix_fmt yuva420p -crf $Crf -b:v 0 `"$OutPath`""
            ...
            while (@($ProcessMatrix.Values | Where-Object { -not $_.HasExited }).Count -ge $MaxProcesses) {
    ...
    $Size = (Get-ChildItem $FinalOutDir -Recurse | Measure-Object -Property Length -Sum).Sum / 1MB
    ...
    Write-Host ("Output size:   {0:N1}M" -f $Size)
    Write-Host ("Directory:     $($FinalOutDir.FullName)")
    Write-Host "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
